...
"""Synergy common internal object model"""
...
class SynergyObject(object):
    ...
    def deserialize(cls, context, entity):
        if not entity.has_key("synergy_object.namespace"):
            raise Exception("synergy_object.namespace nof defined!")

        if not entity.has_key("synergy_object.name"):
            raise Exception("synergy_object.name nof defined!")

        if not entity.has_key("synergy_object.version"):
            raise Exception("synergy_object.version nof defined!")

        if entity["synergy_object.namespace"] != 'synergy':
            raise Exception("unsupported object objtype='%s.%s" % (entity["synergy_object.namespace"], entity["synergy_object.name"]))
    ...
    def serialize(self):
        self.attributes['synergy_object.name'] = self.__class__.__module__ + "." + self.__class__.__name__
