...
#from synergy.openstack.common import local
...
class RequestContext(object):
    ...
    def __init__(self, user_id, project_id, is_admin=None, read_deleted="no",
        ...
        if not timestamp:
            timestamp = datetime.datetime.utcnow() #timeutils.utcnow()
        if isinstance(timestamp, six.string_types):
            timestamp = datetime.datetime.strptime(timestamp, '%Y-%m-%dT%H:%M:%S.%f') #timeutils.parse_strtime(timestamp)
    ...
    def toDict(self):
        return {'user_id': self.user_id,
                ...
                'timestamp': datetime.datetime.strptime(self.timestamp, '%Y-%m-%dT%H:%M:%S.%f'),  #timeutils.strtime(self.timestamp),
