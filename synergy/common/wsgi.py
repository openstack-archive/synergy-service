...
class Server(object):
    ...
    def __init__(self, name, host_name, host_port=8051, threads=1000, application=None, use_ssl=False, ssl_ca_file=None,
                 ssl_cert_file=None, ssl_key_file=None, max_header_line=16384, retry_until_window=30, tcp_keepidle=600,
                 backlog=4096):
    ...
    def start(self):
        ...
        if not self.socket:
            raise RuntimeError("Could not bind to %s:%s after trying for %d seconds" % (
                self.host_host, self.host_port, self.retry_until_window))
