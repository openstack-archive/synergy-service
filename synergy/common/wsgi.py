...
''' % {'subject': subject}]
    """
    print(">>>>>>>>>>>>>>>> Redirect to http://193.206.210.135:8774/v2/558ffcc3c89b433d8a21c61ab484113e")
    start_response('305 Redirect', [("X-Forwarded-For", "193.206.210.135"), ('Location', 'http://193.206.210.135:8774/v2/558ffcc3c89b433d8a21c61ab484113e')])
    return []
    """
        ...
        if application is not None:
            try:
                ...
                for item in self.appiter:
                    yield item
            # if an exception occours we get the exception information and prepare a traceback we can render
            except:
                e_type, e_value, tb = exc_info()
                traceback = ['Traceback (most recent call last):']
                traceback += format_tb(tb)
                traceback.append('%s: %s' % (e_type.__name__, e_value))
                # we might have not a stated response by now. try to start one with the status
                # code 500 or ignore an raised exception if the application already started one.
                try:
            ...
            # wsgi applications might have a close function. If it exists it *must* be called.
...
class Server(object):
    ...
    def __init__(self, name, host_name, host_port=8051, threads=1000, application=None, use_ssl=False, ssl_ca_file=None, ssl_cert_file=None, ssl_key_file=None, max_header_line=16384, retry_until_window=30, tcp_keepidle=600, backlog=4096):
        ...
        tcp_keepidle: sets the value of TCP_KEEPIDLE in seconds for each server socket. Not supported on OS X
        ...
        if use_ssl:
            if not os.path.exists(ssl_cert_file):
                raise RuntimeError("Unable to find ssl_cert_file: %s" % ssl_cert_file)

            if not os.path.exists(ssl_key_file):
                raise RuntimeError("Unable to find ssl_key_file : %s" % ssl_key_file)

            # ssl_ca_file is optional
            if ssl_ca_file and not os.path.exists(ssl_ca_file):
                raise RuntimeError("Unable to find ssl_ca_file: %s" % ssl_ca_file)
    ...
    def start(self):
        ...
        try:
            info = socket.getaddrinfo(self.host_name, self.host_port, socket.AF_UNSPEC, socket.SOCK_STREAM)[0]
            family = info[0]
            bind_addr = info[-1]
        except Exception as ex:
            LOG.error("Unable to listen on %s:%s: %s" % (self.host_name, self.host_port, ex))
            raise ex
        ...
        while not self.socket and time.time() < retry_until:
            try:
                self.socket = eventlet.listen(bind_addr, backlog=self.backlog, family=family)
                if self.use_ssl:
                    self.socket = ssl.wrap_socket(self.socket, **self.ssl_kwargs)
        ...
        if not self.socket:
            raise RuntimeError("Could not bind to %s:%s after trying for %d seconds" % (self.host_host, self.host_port, self.retry_until_window))
    ...
    def _single_run(self, application, sock):
        ...
        eventlet.wsgi.server(sock, application, custom_pool=self.pool, log=WSGILog(LOG), debug=False)
