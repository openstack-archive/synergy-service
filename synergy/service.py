...
from synergy.common import command
...
class ManagerRPC(object):
    ...
    def execute(self, ctx, **args):      
        ...
        if self.managers.has_key(manager_name):
                ...
                try:
                    if len(CONF.get(name).dependences) > 0:
                        for dependence in CONF.get(name).dependences:
                            if manager.dependences.has_key(dependence):
                                continue
                            elif self.managers.has_key(dependence):
                                ...
                                if manager_dep.getStatus() == "ACTIVE":
                                    ...
                                    raise Exception("found cyclic dependence: %s <-> %s" % (name, manager_dep.getName()))
                            else:
...
def main():        
    try:
        ...
        if not cfg.CONF.config_file:
            sys.exit("ERROR: Unable to find configuration file via the default search paths (~/.synergy/, ~/, /etc/synergy/, /etc/) and the '--config-file' option!")
